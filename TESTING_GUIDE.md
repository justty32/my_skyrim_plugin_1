...
> 對應版本：`feature/court-wizard` @ `647e93d`，成果包 `dist/CourtWizardSuite-0.0.1.zip`。
...
## 第 3 步：劇情引擎 + 宮廷大法師 demo（核心）

☐ **3.1** 進遊戲後，**等 48 遊戲小時**（找床睡覺最快），**或**直接按 **`F9`**（立即輪詢劇情計時器，免等）。
☐ **3.2** 預期：跳通知「一封來自龍臨堡的信送到了」，接著彈出 **MessageBox 對話**（領主請你解咒），有兩個按鈕。
☐ **3.3** 按 **「讓我看看那名侍從」** → 預期：面前生成一個 NPC（受詛咒侍從）、log 記 `spawn`、目標 `lift_curse` 變 active。
☐ **3.4** 按 **`F10`**（模擬「對侍從施法解咒」事件）→ 預期：目標完成 → 彈出「詛咒已解」感謝對話。
☐ **3.5** 按 **「舉手之勞」** → 預期：得 500 金（看右下角通知 / 開背包看金幣）、任務重置（log 出現開場訊息）。

**算過**：對話正常彈出、按鈕**對應正確**（按第 1 個就走第 1 個分支）、NPC 有生成、金幣有加。
**特別注意**：按鈕索引對應是程式裏唯一沒法靜態驗證的點（見 REVIEW M-項）——若**按了按鈕 1 卻走了按鈕 2 的結果**，這是 bug，務必回報。
...
- **大致能跑** → 回報哪些過/不過，我據 log 修 in-game 才現形的問題；接著可挑：把煉藥 spike 補 co-save、procgen 換成一套真正對齊的模組化套件、接 `spell_cast_on` 的真實 hook（取代 F10）、或做新功能。
